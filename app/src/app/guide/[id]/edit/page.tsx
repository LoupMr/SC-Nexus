...
export default function EditGuidePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/guides/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.error || (data.authorUsername?.toLowerCase() !== user.username.toLowerCase() && !user.roles?.includes("admin") && !user.roles?.includes("guide"))) {
  ...
  return (
    <>
      <div className="mb-6">
        <Link href={`/guide/${id}`} className="inline-flex items-center gap-2 text-sm text-space-500 hover:text-holo transition-colors mb-4">
          <ArrowLeft className="w-4 h-4" /> Back to guide
        </Link>
        <h1 className="text-2xl font-bold text-space-200 flex items-center gap-2">
          <BookOpen className="w-6 h-6 text-holo" /> Edit guide
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-3xl">
        ...
        <div className="flex gap-3">
          ...
          >
            {busy ? <Loader2 className="w-4 h-4 animate-spin inline" /> : "Save changes"}
          </button>
          <Link href={`/guide/${id}`} className="px-6 py-2.5 bg-space-800/50 border border-glass-border text-space-300 rounded-xl text-sm font-medium hover:bg-space-800">
            Cancel
          </Link>
        </div>
      </form>
